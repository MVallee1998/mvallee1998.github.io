...
Usage:
    ...
    python scripts/bib2hugo.py --clean   # overwrite pages and remove obsolete generated pages

Each BibTeX entry produces content/papers/<section>/<citekey>/index.md.
...
GENERATED_MARKER = '<!-- Generated by scripts/bib2hugo.py; edit the BibTeX source instead. -->'
...
def extract_arxiv_id(fields):
    """Return an arXiv identifier from common BibTeX/Zotero fields."""
    eprint = fields.get('eprint', '').strip()
    if eprint:
        return re.sub(r'(?i)^arxiv:', '', eprint).removesuffix('.pdf')

    candidates = ' '.join(
        fields.get(field, '') for field in ('url', 'doi', 'note')
    )
    match = re.search(
...
def entry_to_markdown(citekey, fields):
    ...
    # editPost URL: prefer arXiv, then DOI, then plain URL
    if eprint:
    ...
    link_part = f'https://arxiv.org/abs/{eprint}' if eprint else (f'https://doi.org/{doi}' if doi else (url if url else ''))
...
def is_generated_page(path):
    """Return whether a page is managed by this script.

    Older generated pages predate GENERATED_MARKER, so recognize those by the
    BibTeX citekey embedded in the page matching its directory name.
    """
    try:
    ...
    return any(citekey == path.parent.name
               for citekey, _fields in parse_bib_string(content))
...
# Map bib filename (stem) → subsection folder under content/papers/
BIB_SECTION = {
    'preprints': 'preprints',
    'articles':  'published',
...
def main():
    ...
    force = '--force' in sys.argv or clean
    ...
    created = updated = skipped = removed = errors = 0
    expected_by_section = {}
    cleanable_sections = set()
    ...
    for bib_path in bib_files:
        ...
        cleanable_sections.add(section_dir)
        expected_by_section.setdefault(section_dir, set()).update(
            citekey for citekey, _fields in entries
        )
    ...
    if clean:
        for section_dir in sorted(cleanable_sections):
            expected = expected_by_section.get(section_dir, set())
            if not section_dir.exists():
                continue

            for out_dir in sorted(p for p in section_dir.iterdir() if p.is_dir()):
                out_file = out_dir / 'index.md'
                if out_dir.name in expected or not out_file.exists():
                    continue
                if not is_generated_page(out_file):
                    print(f'  KEEP  {out_file.relative_to(project_root)} (not recognized as generated)')
                    continue
                ...
                try:
                    ...
                    print(f'  CLEAN {out_file.relative_to(project_root)} (other assets preserved)')
                else:
                    print(f'  CLEAN {out_dir.relative_to(project_root)}')
                removed += 1
